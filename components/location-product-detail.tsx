...
export default function LocationProductDetail({
  product,
  ...
  const related = getLocationRentalProducts()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submit({
        ...
        message: details,
        pagePath: pathname,
        source: `Formulaire location · ${product.category}`,
      });
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {related.length > 0 && (
            <section className="mt-14 border-t border-outline-variant/40 pt-12">
              <h2 className="font-heading mb-6 text-xl font-semibold text-primary sm:text-2xl">
                Autres matériels en location
              </h2>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {related.map((item) => (
                  <Link
                    key={item.slug}
                    href={locationRentalProductPath(item.slug, citySlug)}
                    className="group overflow-hidden rounded-2xl border border-outline-variant/40 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-md"
                  >
              ...
              <div className="mt-8 text-center">
                <Link
                  href={catalogPath}
                  className="inline-flex items-center gap-2 rounded-xl border-2 border-secondary px-6 py-3 text-sm font-semibold text-secondary transition-all hover:bg-secondary/10"
                >
